...
- [x] Variables de entorno configuradas por el usuario (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`).
- [x] Bug real detectado en producción: `@react-pdf/renderer` leía los PNG del logo vía `fs`/`path.join(process.cwd(), ...)` en la función serverless, lo cual falló con `Z_DATA_ERROR` aunque los mismos archivos se servían bien como estáticos. Se corrigió embebiendo las imágenes como base64 en `lib/pdf/logo-assets.ts` (sin lectura de filesystem), enfoque recomendado por la propia documentación de react-pdf para entornos serverless.
- [x] Proyecto de Vercel conectado al repo de GitHub (`davicho21/landing`, rama `main`) — se abandonó el deploy manual vía `deploy_to_vercel` (subida de archivos) porque retransmitir los PNG en base64 a través de la generación del propio modelo demostró ser poco confiable (mismo tipo de corrupción que causó el bug de arriba). El deploy vía git es inmune a ese riesgo.
- **Desviación del plan ya resuelta**: inicialmente no había forma de conectar GitHub ni configurar env vars por herramienta (el MCP de Vercel solo exponía subida directa de archivos); ambas cosas se resolvieron por acción manual del usuario en el dashboard.
